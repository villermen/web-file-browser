...
class Configuration
{
    ...
    protected array $resolvedConfiguration;
    ...
    {
        try {
            $this->resolvedConfiguration = Yaml::parseFile($filePath)['config'];
        } catch (ParseException $exception) {
            throw new ConfigurationException('Unable to process configuration.', previous: $exception);
        }
     ...
     * @throws ConfigurationException Thrown when the directory is inaccessible or its configuration could not be parsed.
     */
    public function getDirectory(string $directory): Directory
    {
        ...
        if (!is_dir($directory)) {
            throw new ConfigurationException(sprintf('Specified directory "%s" does not exist.', $directory));
        }
        ...
        if (!$directoryConfig) {
            throw new ConfigurationException('No configuration could be obtained for the requested directory.');
        }
        ...
        if (!$exactMatch && !$recursive) {
            throw new ConfigurationException('First parent configuration entry encountered is not recursive.');
        }
        ...
        if (!$displayWebpages && !$displayDirectories && !$displayFiles) {
            throw new ConfigurationException('Directory is not able to display anything.');
        }
     ...
     */
    public function isDirectoryAccessible(string $directory, ?string &$reason = null): bool
    {
        try {
